# File: antikythera_portrait_v3.py
# Adjusted layout: top-aligned rings, wide buttons spanning full width
...
class AntikytheraRings(Widget):
    ...
    def update_canvas(self, *args):
        ...
        disk_size = min(w, h)
        x0 = (w - disk_size) / 2
        # Lift the drawing center upward significantly
        y0 = (h - disk_size) / 2
        center = (x0 + disk_size / 2, y0 + disk_size * 0.9)

        radius_px = [270, 307, 345, 395, 432, 470, 510]
        ...
        with self.canvas:
            ...
            for i, angle in enumerate(self.angles):
                ...
                Rectangle(texture=tex, pos=(-size / 2, -size / 2), size=(size, size))
                PopMatrix()
...
class SplitCanvasApp(App):
    def build(self):
        ...
        display = AntikytheraRings(size_hint=(1, 0.65))
        control_area = BoxLayout(orientation='vertical', size_hint=(1, 0.35), padding=[0,10,0,0], spacing=4)
        ...
        date_input = TextInput(text=now.strftime('%Y-%m-%d %H:%M'),
                               size_hint=(0.45, 1), multiline=False, halign='center', font_size='18sp')
        date_label = Label(text='UTC Date/Time:', size_hint=(0.25,1), font_size='18sp', color=(0,0,0,1))
        ...
        # --- Slider row ---
        date_row = BoxLayout(orientation='horizontal', size_hint=(1, None), height=60, spacing=8, padding=[8,0,8,0])
        date_row.add_widget(date_label)
        date_row.add_widget(date_slider)
        date_row.add_widget(date_input)

        # --- Buttons row (equal width) ---
        date_controls = BoxLayout(orientation='horizontal', size_hint=(1, None), height=65, spacing=0, padding=[0,0,0,0])
        controls = [
            ("Now", None),
        ...
        for i, (label, kwargs) in enumerate(controls):
            b = Button(text=label, size_hint_x=1/len(controls))
            if kwargs is None:
        ...
        control_area.add_widget(date_row)
        control_area.add_widget(date_controls)

        root.add_widget(display)
        root.add_widget(control_area)
